...
const Login = ({ setAuth }) => {
  ...
  const FIXED_USERNAME = 'yesadm';
  const FIXED_PASSWORD = 'vocarcot';
  ...
  const onFinish = (values) => {
    ...
    if (email === FIXED_USERNAME && password === FIXED_PASSWORD) {
      ...
      api.error('Invalid ID or Password');
    }
  };
    ...
    <div className="login-container">
      ...
      <Card className="login-card">
        ...
        <Form layout="vertical" onFinish={onFinish}>
          <Form.Item
            label="Admin Unique ID"
            name="email"
            rules={[{ required: true, message: "Enter your Admin Unique ID" }]}
          >
            <Input />
          </Form.Item>
          <Form.Item
